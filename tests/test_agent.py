...
def _no_auto_retrieval(monkeypatch):
    """
    run_turn() always runs a local, automatic knowledge-base lookup for the
    user's message before calling the LLM (a speed optimization — see
    agent/agent.py). That lookup hits the real vector store, which would
    make these orchestration tests depend on whatever happens to be
    ingested on this machine. Stub it out by default so tests here are only
    exercising the tool-calling loop itself; tests that care about the
    auto-lookup content can override this fixture's patches locally.
    """
    monkeypatch.setattr("agent.agent.retrieve", lambda query: [])
    monkeypatch.setattr("agent.agent.classify_retrieval", lambda results: "not_found")
